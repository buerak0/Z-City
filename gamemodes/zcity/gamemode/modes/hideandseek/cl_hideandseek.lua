...
local song
local songfade = 0
net.Receive("hideandseek_start", function()
...
local teams = {
	[0] = {
		objective = "Find the hiders before evacuations and backup arrive!",
		name = "Seeker",
...
function MODE:HUDPaint()
	if zb.ROUND_START + 31 > CurTime() then
		...
		draw.SimpleText( "Seekers spawn in "..string.FormattedTime(zb.ROUND_START + 90 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.95, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText( "Seekers spawn in "..string.FormattedTime(zb.ROUND_START + 90 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.95) - 2, color, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		local fade = math.Clamp(zb.ROUND_START + 7.5 - CurTime(), 0, 1)
		surface.SetDrawColor(0, 0, 0, 255 * fade)
		surface.DrawRect(-1, -1, ScrW() + 1, ScrH() + 1)
	end
	...
	if zb.ROUND_START + 8.5 > CurTime() then
		...
		draw.SimpleText("Hide and Seek", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local CreateEndMenu
net.Receive("hideandseek_roundend", function() CreateEndMenu(net.ReadBool()) end)
